...
function bubbleChart() {
	...
	var forceStrength = 2; // previously 1
	...
	var chart = function chart(selector, rawData) {
	   ...
	   // Use the max total_amount in the data as the max in the scale's domain
	    ...
	    bubblesLinks = svg.append("g")
			...
			.classed("canDisappear", function(d){
  				if (d.value==1)
  					return true;
  				else
  					return false;
  		...
  		if(d.subgroup>0){
			...
			if(filtered_nodes.length>0){
				...
				if(parseFloat(d.value)>=0)
					document.getElementById("bubbleOrientation").innerHTML = "shortage";
				else
					document.getElementById("bubbleOrientation").innerHTML = "surplus";

				document.getElementById("selBubble").innerHTML = d.name;
	...
	function showUsage(data,colorNode){
		...
		var usageBubbles = usageCircles.selectAll("g")
			...
			.attr("transform",function(d,i){
				i++;
				return "translate("+i*width/6 + ","+height_usage/4 + ")"
			});

			usageBubbles.append("circle")
			.attr("r", 25/**function(d){return (radiusUsage(parseFloat(d.value)));}**/)
			...
			.call(wrap,wrapWidth);**/
			usageBubbles.append("image")
			    .attr("xlink:href", function(d){
						var key=key=d.name.replace(/ /g,"_");
